...
program
  ...
  .option(
    "--base-url <baseUrl>",
    "The base url to crawl, eg. https://docs.aws.amazon.com/"
  )
  .option(
    "--start-paths <paths...>",
    "The relative paths in the site to start crawling, eg /lambda"
  )
  .option(
    "--keywords [keywords...]",
    "Optional keywords to filter the urls to visit, eg lambda/latest/dg"
  );
...
(async () => {
  ...
  const cfnExports =
    await paginatedRequest<AWS.CloudFormation.Types.ListExportsInput>(
  ...
  const startCrawlFunctionArnExport = cfnExports.find(
    (exp) => exp.Name === "StartCrawlFunctionArn"
  );
  ...
  if (startCrawlFunctionArnExport) {
    ...
    if (response.Payload) {
      const { stateMachineExecutionArn } = JSON.parse(
    ...
    console.error(
      "Couldn't find export with name StartCrawlFunctionArn. Have you deployed yet?"
    );
  }
})();
