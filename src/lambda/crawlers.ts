...
import {
  completeCrawl,
  KendraDataSourceDetails,
} from "./steps/5_completeCrawl";
...
export const readQueuedUrlsHandler = async (event: any, context: any) => {
  ...
  if (!HISTORY_TABLE_NAME) {
    throw new Error(
      "Environment not configured correctly. HISTORY_TABLE_NAME must be specified"
    );
  }
 ...
 * context database
 */
export const crawlPageAndQueueUrlsHandler = async (
  event: any,
  context: any
) => {
  console.log(event, context);
  ...
  return await crawlPageAndQueueUrls(
    path,
    crawlContext,
    DATA_SOURCE_BUCKET_NAME
  );
...
export const completeCrawlHandler = async (event: any, context: any) => {
  ...
  const kendraDataSourceDetails: KendraDataSourceDetails | undefined =
    KENDRA_INDEX_ID && KENDRA_DATA_SOURCE_ID
      ? {
