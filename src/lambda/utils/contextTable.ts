...
import {
  CONTEXT_TABLE_READ_CAPACITY,
  CONTEXT_TABLE_WRITE_CAPACITY,
  PARALLEL_URLS_TO_SYNC,
 ...
 */
export const createContextTable = async (
  target: CrawlInputWithId,
...
export const queuePaths = async (contextTableName: string, paths: string[]) => {
  ...
  for (const pathGroup of _.chunk(paths, 20)) {
    ...
    await Promise.all(
      pathGroup.map(async (newPath) => queuePath(contextTableName, newPath))
    );
  }
};
 ...
 */
export const readBatchOfUrlsToVisit = async (
  contextTableName: string
): Promise<string[]> => {
  // Get all paths that have not been visited, up to our limit of PARALLEL_URLS_TO_SYNC
  const toVisitEntries = await dynamodbPaginatedRequest(
    ddbDoc.query.bind(ddbDoc),
    {
      TableName: contextTableName,
      ConsistentRead: true,
 ...
 */
export const markPathAsVisited = async (
  contextTableName: string,
  path: string
) => {
